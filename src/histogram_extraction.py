...
def extract_histograms_streaming(
    ...
    output_path: Optional[str] = DataPath.MODEL_DIR + r"/histogram/histogram_kde_streaming.npz",
    ...
    for idx, row in tqdm(df.iterrows(), total=total_images, desc="Processing images"):
        try:
            ...
            features_selected = features[..., feature_ids]  # Shape: (H, W, num_features)
            
            #! Flatten for histogram computation
            h, w, f = features_selected.shape
            features_flat = features_selected.reshape(-1, num_features)  # (H*W, num_features)
    ...
    result = {
        ...
        'kde': kde,
        'bin_edges': np.tile(bin_edges, (num_features, 1)),  # (num_features, num_bins + 1)
...
def visualize_kde(
    ...
    for class_id in range(num_classes):
        class_name = ClassInfo.CLASS_NAMES.get(class_id, f"Class_{class_id}")
        plt.plot(bin_centers, kde[class_id, feature_idx, :], label=class_name, linewidth=2)
...
if __name__ == "__main__":
    ...
    loaded_data = load_histograms(f"{DataPath.MODEL_DIR}histogram/histogram_kde_streaming.npz")
