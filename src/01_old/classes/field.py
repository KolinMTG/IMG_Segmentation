"""
Field class detection (Class 0) using probabilistic classifier.

Strategy:
- Extract comprehensive feature vector for each pixel
- Compute P(pixel | Field) using trained GMM
...
def _load_model(model_dir: str = "data/models/"):
    """
    Load trained Field model from disk.
    
    Args:
        model_dir: Directory containing model files
        
    Returns:
        Trained GMM model for Field class
    """
    global _field_model
    
    if _field_model is None:
        class_name = ClassInfo.CLASS_NAMES[0]  # "Field"
        model_path = Path(model_dir) / f"{class_name.lower()}_model.pkl"
        
        if model_path.exists():
            with open(model_path, 'rb') as f:
                _field_model = pickle.load(f)
        else:
...
def process_field(img: np.ndarray) -> np.ndarray:
    """
    Generate field likelihood score map using probabilistic model.
    
    Detection strategy:
    1. Extract comprehensive features (color, texture, gradients, indices)
    2. Evaluate P(features | Field) using trained GMM
    ...
    # ! Compute log-likelihood P(x | Field)
