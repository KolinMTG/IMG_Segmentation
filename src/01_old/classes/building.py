"""
Building class detection (Class 1) using probabilistic classifier.

Strategy:
- Extract comprehensive feature vector for each pixel
- Compute P(pixel | Building) using trained GMM
...
def _load_model(model_dir: str = "data/models/"):
    """
    Load trained Building model from disk.
    
    Args:
        model_dir: Directory containing model files
        
    Returns:
        Trained GMM model for Building class
    """
    global _building_model
    
    if _building_model is None:
        class_name = ClassInfo.CLASS_NAMES[1]  # "Building"
        model_path = Path(model_dir) / f"{class_name.lower()}_model.pkl"
        
        if model_path.exists():
            with open(model_path, 'rb') as f:
                _building_model = pickle.load(f)
        else:
...
def process_building(img: np.ndarray) -> np.ndarray:
    """
    Generate building likelihood score map using probabilistic model.
    
    Detection strategy:
    1. Extract comprehensive features (color, texture, gradients, indices)
    2. Evaluate P(features | Building) using trained GMM
    ...
    # ! Compute log-likelihood P(x | Building)
