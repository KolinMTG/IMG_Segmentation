"""
Woodland class detection (Class 2) using probabilistic classifier.

Strategy:
- Extract comprehensive feature vector for each pixel
- Compute P(pixel | Woodland) using trained GMM
...
_woodland_model = None


def _load_model(model_dir: str = "data/models/"):
    """
    Load trained Woodland model from disk.
    
    Args:
        model_dir: Directory containing model files
        
    Returns:
        Trained GMM model for Woodland class
    """
    global _woodland_model
    
    if _woodland_model is None:
        class_name = ClassInfo.CLASS_NAMES[2]  # "Woodland"
        model_path = Path(model_dir) / f"{class_name.lower()}_model.pkl"
        
        if model_path.exists():
            with open(model_path, 'rb') as f:
                _woodland_model = pickle.load(f)
        else:
            raise FileNotFoundError(
                f"Model not found: {model_path}. Run train_classifier.py first."
            )
    
    return _woodland_model


def process_woodland(img: np.ndarray) -> np.ndarray:
    """
    Generate woodland likelihood score map using probabilistic model.
    
    Detection strategy:
    1. Extract comprehensive features (color, texture, gradients, indices)
    2. Evaluate P(features | Woodland) using trained GMM
    ...
    # ! Compute log-likelihood P(x | Woodland)
