"""
Road class detection (Class 4) using probabilistic classifier.

Strategy:
- Extract comprehensive feature vector for each pixel
- Compute P(pixel | Road) using trained GMM
...
def _load_model(model_dir: str = "data/models/"):
    """
    Load trained Road model from disk.
    
    Args:
        model_dir: Directory containing model files
        
    Returns:
        Trained GMM model for Road class
    """
    global _road_model
    
    if _road_model is None:
        class_name = ClassInfo.CLASS_NAMES[4]  # "Road"
        model_path = Path(model_dir) / f"{class_name.lower()}_model.pkl"
        
        if model_path.exists():
            with open(model_path, 'rb') as f:
                _road_model = pickle.load(f)
        else:
...
def process_road(img: np.ndarray) -> np.ndarray:
    """
    Generate road likelihood score map using probabilistic model.
    
    Detection strategy:
    1. Extract comprehensive features (color, texture, gradients, indices)
    2. Evaluate P(features | Road) using trained GMM
    ...
    # ! Compute log-likelihood P(x | Road)
