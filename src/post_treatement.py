...
def posttreat_pipeline(mask: np.ndarray, **kwargs) -> np.ndarray:
    """
...
def _process_roads_aggressive(
    ...
    road_mask_clean = remove_small_objects(
        road_mask.astype(bool), min_size=min_length, connectivity=2
    ).astype(np.uint8)
    ...
    for i, comp1 in enumerate(major_components):
        for comp2 in major_components[i + 1 :]:
            ...
            if angle_diff <= angle_tolerance:
                log.info(
                    f"Bridging components: distance={distance:.1f}px, angle_diff={angle_diff:.1f}°"
                )
...
def _extend_roads_to_borders(
    ...
    for comp in major_components:
        ...
        log.info(
            f"Extending road component at ({int(centroid[0])}, {int(centroid[1])})"
        )
...
def _detect_long_road_axes(
    road_mask: np.ndarray, min_length: int = 100
...
def _connect_road_fragments(skeleton: np.ndarray, max_distance: int = 25) -> np.ndarray:
    """
...
def _process_buildings(
    ...
    for region in regions:
        ...
        if len(contours) > 0:
            ...
            if len(contour) >= 5:
                ...
                if (
                    rotated_area > 0
                    and region.area / rotated_area > regularization_threshold
                ):
    ...
    if expansion_pixels > 0:
        kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (expansion_pixels * 2 + 1, expansion_pixels * 2 + 1)
        )
    ...
    for cls in natural_classes:
        ...
        for region in regions:
            if region.area < min_area:
                ...
                neighbor_classes = neighborhood[
                    ~region_pixels[minr_pad:maxr_pad, minc_pad:maxc_pad]
                ]
