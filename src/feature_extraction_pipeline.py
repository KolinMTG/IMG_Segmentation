...
def downsample_image(img: np.ndarray, fraction: float) -> np.ndarray:
    ...
    if fraction >= 1.0:
        log.warning(
            "Trying to downsample with fraction >= 1.0, returning original image"
        )
        return img
...
def compute_gradient_anisotropy_fast(
    gray: np.ndarray, window_size: int = 7
) -> np.ndarray:
    """
    ...
    Ixx = cv2.boxFilter(
        sobel_x * sobel_x, -1, (window_size, window_size), normalize=True
    )
    Iyy = cv2.boxFilter(
        sobel_y * sobel_y, -1, (window_size, window_size), normalize=True
    )
    Ixy = cv2.boxFilter(
        sobel_x * sobel_y, -1, (window_size, window_size), normalize=True
    )
