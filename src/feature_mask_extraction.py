    ...
    try:
        ...
        if augmentation_ratio == 0:
            ...
            record = _process_single_pair(
                img, mask, img_id, "original",
                feature_dir, mask_dir,
                normalize, downsample_fraction,
                seed=None
            )
            ...
            for aug_idx in range(augmentation_ratio):
                ...
                record = _process_single_pair(
                    aug_img, aug_mask, img_id, aug_id,
                    feature_dir, mask_dir,
                    normalize, downsample_fraction,
                    seed=aug_seed
                )
...
def extract_features_with_augmentation(
    ...
    if augmentation_ratio > 0:
        ...
        log.info(f"Expected output samples: {expected_outputs} "
    ...
    if augmentation_ratio > 0:
        original_count = sum(1 for r in all_metadata_records if r['variant'] == 'original')
