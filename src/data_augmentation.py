...
def augment_segmentation_data(
    ...
    critical_mask = _create_critical_mask(mask, critical_class_ids) if use_critical_sampling else None
...
def _create_critical_mask(mask: np.ndarray, critical_class_ids: List[int]) -> np.ndarray:
...
def _augment_standard(
    ...
    # Random zoom
    zoom_factor = np.random.uniform(ProcessingConfig.ZOOM_RANGE[0], ProcessingConfig.ZOOM_RANGE[1])
...
def _zoom_pair(
    ...
    if zoom_factor > 1.0:
        ...
        cropped_mask = resized_mask[start_y:start_y+target_h, start_x:start_x+target_w]
...
def _adjust_brightness_contrast(img: np.ndarray) -> np.ndarray:
    ...
    brightness_delta = np.random.uniform(ProcessingConfig.BRIGHTNESS_RANGE[0], ProcessingConfig.BRIGHTNESS_RANGE[1])
    img_adjusted = img + brightness_delta
    
    # Contrast adjustment
    contrast_factor = np.random.uniform(ProcessingConfig.CONTRAST_RANGE[0], ProcessingConfig.CONTRAST_RANGE[1])
