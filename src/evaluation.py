...
def compute_confusion_matrix(gt_masks: np.ndarray, pred_masks: np.ndarray, 
...
def plot_confusion_matrix(confusion_matrix: np.ndarray, class_names: Dict[int, str],
                          output_path: str, normalized: bool = True) -> None:
    """
...
def plot_metric_bars(metrics: np.ndarray, class_names: Dict[int, str],
    ...
    """
    labels = [class_names[i] for i in range(len(class_names))]
    colors = [np.array(ClassInfo.CLASS_COLORS[i]) / 255.0 for i in range(len(class_names))]
...
def plot_metrics_summary(metrics_dict: Dict, class_names: Dict[int, str],
...
def generate_report(metrics_dict: Dict, class_names: Dict[int, str],
                   num_images: int, output_path: str) -> None:
    """
    ...
    for i in range(len(class_names)):
        ...
        report_lines.append(f"  IoU:              {metrics_dict['iou_per_class'][i]:.4f}")
        report_lines.append(f"  Dice (F1-score):  {metrics_dict['dice_per_class'][i]:.4f}")
...
def evaluate_predictions(
    ...
    gt_files = sorted([f for f in os.listdir(ground_truth_dir) 
                       if f.endswith('.png')])
    ...
    for gt_file, pred_file in zip(gt_files, pred_files):
        ...
        gt_mask = np.array(Image.open(os.path.join(ground_truth_dir, gt_file)).convert("L"))
        pred_mask = np.array(Image.open(os.path.join(predictions_dir, pred_file)).convert("L"))
        
        #! Verify shapes match
        if gt_mask.shape != pred_mask.shape:
            raise ValueError(f"Shape mismatch for {gt_file} vs {pred_file}: "
    ...
    plot_confusion_matrix(
        total_confusion, ClassInfo.CLASS_NAMES,
        os.path.join(output_path, "confusion_matrix.png"),
        normalized=True
    )
    
    plot_metric_bars(
        iou_per_class, ClassInfo.CLASS_NAMES, "IoU",
        os.path.join(output_path, "iou_per_class.png")
    )
    
    plot_metric_bars(
        dice_per_class, ClassInfo.CLASS_NAMES, "Dice Score",
        os.path.join(output_path, "dice_per_class.png")
    )
    
    plot_metrics_summary(
        metrics_dict, ClassInfo.CLASS_NAMES,
...
if __name__ == "__main__":
    ...
    evaluate_predictions(
        ground_truth_dir=DataPath.LABEL_TEST,
        predictions_dir=DataPath.UNET_INFERENCE_DIR,
        output_path=r"data/results/evaluation_report/unet_inference/"
    )