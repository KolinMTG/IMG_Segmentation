...
def sample_balanced_pixels(
    csv_path: str, samples_per_class: int = 50000, max_images: int = 500
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Loading images"):
        ...
        try:
            features, label = load_feature_and_label(
                row["feature_path"], row["label_path"]
            )
    ...
    for class_id, feat_list in class_features.items():
        ...
        if len(class_feat) > samples_per_class:
            indices = np.random.choice(
                len(class_feat), size=samples_per_class, replace=False
            )
...
def analyze_global_distributions(features: np.ndarray, output_dir: Path) -> None:
    ...
    for i in range(n_features):
        ...
        print(
            f"{FeatureInfo.FEATURE_NAMES[i]:20s}: "
...
def analyze_pca(features: np.ndarray, labels: np.ndarray, output_dir: Path) -> None:
    ...
    ax1.bar(
        range(1, len(explained_var) + 1), explained_var, alpha=0.6, edgecolor="black"
    )
    ...
    ax.set_xlabel(
        f"PC1 ({explained_var[0]:.1%} variance)", fontweight="bold", fontsize=12
    )
    ax.set_ylabel(
        f"PC2 ({explained_var[1]:.1%} variance)", fontweight="bold", fontsize=12
    )
    ...
    ax.set_zlabel(
        f"PC3 ({explained_var[2]:.1%} variance)", fontweight="bold", fontsize=12
    )
